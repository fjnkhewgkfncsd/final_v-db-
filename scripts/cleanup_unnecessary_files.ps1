...
Write-Host "🗑️  Starting safe file cleanup..." -ForegroundColor Yellow
...
function Remove-SafeFiles {
    ...
    Write-Host "🗑️  Removing $category files..." -ForegroundColor Cyan
    $removed = 0
    ...
    Write-Host "   📊 Removed $removed/$($files.Count) $category files" -ForegroundColor Gray
    Write-Host ""
}
...
Write-Host "🤔 Docker files (only remove if not using Docker):" -ForegroundColor Yellow
...
Write-Host ""
Write-Host "🗑️  Old backup files cleanup (June backups):" -ForegroundColor Yellow
...
if ($oldBackups) {
    ...
    Write-Host "   💡 Run this to remove them: Remove-Item 'backups\*2025-06-*' -Force" -ForegroundColor Cyan
} else {
    Write-Host "   ✅ No old backup files found" -ForegroundColor Green
}

Write-Host ""
Write-Host "✅ Safe file cleanup completed!" -ForegroundColor Green
Write-Host "💡 Review the removed files and commit changes to Git if satisfied." -ForegroundColor Cyan
